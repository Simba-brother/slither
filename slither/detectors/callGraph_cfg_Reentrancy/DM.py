...
class DM_updateETH():
    def __init__(self, function):
        ...
        self.KEY = 'updateETH'
        self.explore(function.entry_point, [])
        self.function.context[self.KEY] = True
    ...
    def _can_callback(self, irs): # irs = node.irs + slithir_operations
        ...
        for ir in irs:
            ...
            if isinstance(ir, HighLevelCall) and not isinstance(ir, LibraryCall):
                ...
                return True
        return False
    def explore(self, node, visited, skip_father=None):
        if node in visited:
            return
        ...
        # 'write' returns the variable read
        # 'write_prior_calls' returns the variable write prior a call
        fathers_context = {'send_eth': set(), 'calls': set(), 'read': set(), 'read_prior_calls': {}}
        for father in node.fathers:
            if self.KEY in father.context:  # 'updateETH'
